...
class ProjectUpdater
{
    ...
    */
    ...
    function getProjectsListFromInput($UserQuery){
        ...
        if (sizeof($names)==1) {
            $sth = "SELECT * FROM projetOk GROUP BY name HAVING name LIKE '%" . $fname . "%' OR lname LIKE '%" . $fname . "%' OR fname LIKE '%" . $fname . "%' ";
        }

        if (sizeof($names)==2){
            $lname = $names[1];
            $sth = "SELECT * FROM projetOk GROUP BY name HAVING fname LIKE '%" . $fname . "%' OR lname LIKE '%" . $lname . "%' OR fname LIKE '%" . $lname . "%' OR lname LIKE '%" . $fname . "%' ";
        }
        #This send the request to the database and returns a list
    ...
    function addProject1(Project $project){
        ...
        $askingHelp = $project->getAskingHelp();
        $descri = $project->getDescription();
        $git = $project->getGithub();
        $slack = $project->getSlack();
        $img = $project->getImg();
